import {
  ...
  export class WishesService {
    ...
    async findOne(id: number) {
      ...
      if (!result) throw new NotFoundException('Your wish has not been found.');
      return result;
    }
    ...
    async update(userId: number, id: number, updateWishDto: UpdateWishDto) {
      ...
      if (wish.owner.id !== userId)
        throw new UnauthorizedException('Сannot change the wishes of others.');
      if (wish.raised > 0 && updateWishDto.price)
        throw new ForbiddenException(
          'Money has already been chipped on this wish.',
        );
    ...
    async remove(userId: number, id: number) {
      const wish = await this.findOne(id);
      if (wish.owner.id !== userId)
        throw new UnauthorizedException('Сannot delete the wishes of others.');
      if (wish.raised > 0)
        throw new ForbiddenException(
          'Money has already been chipped on this wish.',
        );
    ...
    async copy(userId, id: number) {
      ...
      try {
        ...
        throw new InternalServerErrorException('A server error has occurred.');
      } finally {
